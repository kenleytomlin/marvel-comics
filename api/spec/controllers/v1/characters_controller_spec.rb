  ...
  describe "GET #index" do
    ...
    context "when the request is a success" do
      context "when there are query params" do
        before :each do
          expect(API_CLIENT).to receive(:search_characters).with({ :page => 0, :nameStartsWith => 'Wolv' }).and_return({
            :status => 200,
        ...
        it "returns the correct response" do
          ...
          expect(response.body).to eql({ "results": [fake_character], "pagination": { "currentPage": 0, "totalPages" => 1, "isLast": true, "totalElements": 1 } }.to_json)
        end
      end

      context "when there are no query params" do
        before :each do
          expect(API_CLIENT).to receive(:search_characters).with(no_args).and_return({
            :status => 200,
            :characters => [fake_character],
            :pagination => {
              :currentPage => 0,
